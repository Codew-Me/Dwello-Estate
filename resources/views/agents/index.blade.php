            ...
            @forelse($agents as $agent)
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition duration-300">
                    ...
                    <div class="relative h-64">
                        @if($agent->image)
                            <img src="{{ asset('storage/' . $agent->image) }}" alt="{{ $agent->name }}" class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full bg-gray-200 flex items-center justify-center">
                                <div class="w-24 h-24 bg-dwello-brown rounded-full flex items-center justify-center">
                                    <span class="text-white text-2xl font-bold">{{ substr($agent->name, 0, 1) }}</span>
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    <!-- Agent Details -->
                    <div class="p-6">
                        <h3 class="text-xl font-bold text-dwello-brown mb-2">{{ $agent->name }}</h3>
                        <p class="text-gray-600 mb-4">{{ $agent->specialization ?? 'Real Estate Agent' }}</p>
                        
                        <div class="flex items-center mb-4">
                            <svg class="w-5 h-5 text-yellow-400 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"></path>
                            </svg>
                            <span class="text-sm text-gray-600">{{ $agent->experience_years }}+ years experience</span>
                        </div>

                        <p class="text-gray-600 text-sm mb-4 line-clamp-3">{{ $agent->bio }}</p>
                        ...
                        <div class="space-y-2">
                            <div class="flex items-center">
                                ...
                                <span class="text-sm text-gray-600">{{ $agent->email }}</span>
                            </div>
                            <div class="flex items-center">
                                <svg class="w-4 h-4 text-gray-400 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
                                </svg>
                                <span class="text-sm text-gray-600">{{ $agent->phone }}</span>
                            </div>
                        </div>

                        <div class="mt-6">
                            <a href="{{ route('agents.show', $agent) }}" class="w-full bg-dwello-brown text-white py-2 px-4 rounded-lg hover:bg-opacity-90 transition duration-300 text-center block">
                                View Profile
                            </a>
                        </div>
                    </div>
